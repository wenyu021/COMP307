...
if __name__ == '__main__':
    ...
    # add bias
    initial_hidden_layer_bias = [-0.02, -0.2]
    initial_output_layer_bias = [-0.33, 0.26, 0.06]

    nn = Neural_Network(n_in, n_hidden, n_out, initial_hidden_layer_weights, initial_output_layer_weights,
                        learning_rate, initial_hidden_layer_bias, initial_output_layer_bias)
    ...
    n = len(test_desired)
    for i in range(n):
        if test_desired[i] == test_predict[i]:
            correct += 1


    test_accuracy = float(correct) / n
    print('\nAfter testing:')
